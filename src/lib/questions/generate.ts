...
/**
 * Core "generate N MCQ questions" logic, shared by the client-facing
 * /api/questions route (practice, AI Battle setup) and any server-side
 * caller that needs questions without an internal HTTP round-trip (e.g. the
 * friend-battle-invite accept flow). Unlike the route handler, this does NOT
 * persist generated questions to the general `questions` bank — callers that
 * want that (like the route handler) do it themselves as a separate step.
 */
export async function generateQuestions(params: GenerateQuestionsParams): Promise<GeneratedQuestion[]> {
  const { subject, topicName, difficulty, count, classLevel, board } = params;

  // Free-text topic field — same safety gate as the AI Tutor, applied
  // before any DB lookup or model call.
  if (topicName?.trim()) {
    const moderation = await moderateText(topicName);
    if (moderation.blocked) {
      throw new Error("That topic isn't allowed here. Please enter a school curriculum topic.");
    }
  }
  ...
  if (count <= BATCH_SIZE) {
    ...
    results.forEach((r, i) => {
      if (r.status === "rejected") {
        console.error(`[generateQuestions] batch ${i} failed:`, r.reason);
      }
    });
